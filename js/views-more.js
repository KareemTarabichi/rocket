...
const KB = [
  ...
  {id:'k12', title:'Admin: managing members and logins', category:'Admin', roles:[], summary:'For whoever holds admin rights — usually someone on Tech.', steps:['Invite members from Admin with their @aus.edu email, club role and team. Rocket shows a temporary password to send them; they choose their own at first sign-in.','Change someone’s role or team from Admin; their sections and permissions update the next time the page loads.','Disable a login to lock someone out but keep their history. Remove a member only when their records should become unassigned.','If someone forgets their password, open them in Admin and tap Reset password, then send them the new temporary one.','Keep at least two admins so the club never gets locked out.']},
  {id:'k11', title:'Moving an idea through the pipeline', category:'Ideas', roles:[], summary:'Submitted → Under Review → Approved → In Progress → Completed.', steps:['Give every idea an owner, a team and one clear next step.','The next step appears in Deadlines for the owner and collaborators.','Completing that step marks the idea Completed; reopening restores its earlier stage.']}];
